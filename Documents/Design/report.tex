...
\requirement{FR}{Low}{% FR 1.3
  Moving objects within the game environment must not pass through solid geometry.
}

\requirement{FR}{Low}{% FR 1.4
  NPCs must demonstrate an ability to recognise whether a specified object is visible from their perspective.
}
...
\requirement{FR}{High}{% FR 1.6
  NPCs must have the ability to construct barricades, ideally using resource objects (FR 1.5).
}
...
\requirement{FR}{Med}{% FR 1.8
  A player must be able to instruct selected agents (FR 1.7) to move to a chosen location.
}

\requirement{FR}{Med}{% FR 1.9
  A player must be able to instruct selected agents (FR 1.7) to block off a specified area with a barricade.
}

\requirement{FR}{High}{% FR 1.10
...
\requirement{FR}{High}{% FR 2.4
  Human agents must possess the ability to autonomously construct barricades to block off areas specified by a player (using FR 1.9).
}
\end{requirements}
