...
The main deliverable can be broken down into a set of core functional and non-functional requirements, each with a relative priority rating.

\subsubsection{Functional}
\begin{requirements}{FR}
  \requirement{Nav}{NPCs must be able to navigate between any two positions in the game environment if a path exists.}{High}
  \requirement{Col}{Moving objects within the game environment must not pass through solid geometry.}{High}
  \requirement{Vis}{NPCs must demonstrate an ability to recognise whether a specified object is visible from their perspective.}{High}
  \requirement{Res}{Game objects representing a resource used when constructing barricades must exist within the game environment.}{High}
  \requirement{Bar}{NPCs must have the ability to construct barricades using the resource objects added for FR4.}{High}
\end{requirements}
